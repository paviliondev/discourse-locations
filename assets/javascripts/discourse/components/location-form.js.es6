...
export default Component.extend({
  ...
  showAddress(showInputFields, inputFields) {
    return (
      !showInputFields ||
      (showInputFields &&
        inputFields.filter((f) => f !== "coordinates").length > 0)
    );
  },
  ...
  keyDown(e) {
    if (this.get("showGeoLocation") && e.keyCode === 13) {
      this.send("locationSearch");
    }
  },
  ...
  searchLabel: (locationGeocoding) =>
    I18n.t(`location.geo.btn.${locationGeocoding}`),
  ...
  actions: {
    ...
    locationSearch() {
      const inputFields = this.get("inputFields");
      const request = this.getProperties(
        inputFields.concat(["countrycode", "context"])
      );
